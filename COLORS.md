...
| Role | Hex | Where used |
|---|---|---|
| `canvas` | `#100E11` | Ghostty `background`; tmux status bar, borders, message line, all window-status states, and active pane when single-pane or zoomed; tmux `message-style`/`message-command-style` `fill=` (required on next-3.7 so the command-prompt repaints the full line — see Cross-tool notes); mm-sidebar active-tab chip fg. Tmux derives its darker inactive-pane surface from this role (30% toward black; `#0b0a0c` for Vague) and exposes it as `@color-surface-inactive`; near-black palettes that cannot clear the distinctness floor use the midpoint between canvas and `surface-active` instead. **Exposed as the tmux user option `@color-canvas`** — added so a pane running its own TUI can read the hex instead of hardcoding it. The ~15 pre-existing inline `#100E11` literals in `tmux.conf` predate the option and could migrate to it (purely mechanical, not yet done). |
| `surface-active` | `#1B1A21` | Source for tmux's split-pane active bg, only when a window has 2+ panes and is not zoomed. The tmux adapter darkens it 3% toward black (`#1a1920` for Vague) as `@color-surface-pane-active`; single-pane/zoomed panes and active-border bg stay `canvas`. Focus changes synchronously repaint pane contents between this active surface and `@color-surface-inactive`; the visible transition is retained intentionally for focus contrast. The canonical role remains available unchanged to nvim/Pi/LazyGit consumers. |
...
| Role | Hex | Where used |
|---|---|---|
| `accent-primary` (rose) | `#d8647e` | tmux `@color-accent-primary` — all stars in inactive pane footers (labeled and unlabeled); tmux ephemeral session indicator; Ghostty ANSI 1; ohmyposh path segment; nvim lualine `replace`-mode status/location block; mm-sidebar agents-glance `!P`/`!W` (awaiting-permission / waiting) |
