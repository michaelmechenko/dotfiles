...
| `accent-tertiary` (dusty pink) | `#bb9dbd` | tmux `@color-accent-tertiary` — active labeled text; active unlabeled marker stars (all normal stars and the center `*─*─*` zoomed trio; non-center zoomed stars remain lavender); Ghostty ANSI 2; ohmyposh transient prompt + git segment; nvim lualine `visual`-mode status/location block; mm-sidebar agents-glance `~~` (thinking) |
