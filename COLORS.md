...
| `surface-active` | `#1B1A21` | Shared lifted surface for nvim/Pi/LazyGit consumers. Tmux keeps the canonical role unchanged but aliases `@color-surface-pane-active` to `canvas`, so focused panes—including split panes—match the terminal background; inactive panes use the derived `@color-surface-inactive`. Focus changes synchronously repaint between those two surfaces, retaining a subtle focus cue. |
