...
if echo "$CONTEXT" | grep -qiE "$STRONG_PATTERNS"; then
    cat << 'EOF'
This session involved fixes or discoveries. Update memory files in ~/.config/claude/rules/ if you learned something non-obvious:
...
EOF
    ...
    echo 'If you learned something non-obvious this session, update the relevant memory file in ~/.config/claude/rules/.'
fi
