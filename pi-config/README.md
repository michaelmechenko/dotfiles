...
`tool-display/` supplies the shared framing and `ctrl+shift+o` result-detail toggle used by the restored `read`/`bash` and diff renderers, **and** a guarded host-level decorator over Pi’s `ToolExecutionComponent` that adds the same status-marker prefix and call/result divider to every tool that still uses Pi’s default `Box` shell (the built-in `find`/`grep`/`ls` and every extension tool that does not set `renderShell: "self"`). The decorator never touches `renderShell: "self"` tools, never re-implements any tool’s renderer, and falls back to Pi’s unmodified render on any error or unrecognized component shape. Other custom tools use Pi's default shell and result presentation where the decorator applies.
