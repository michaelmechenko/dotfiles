...
| `tmux/` | `tmux` tool and `/tmux` / `/tmux:cat` commands for extension-owned jobs in the active tmux session. It exposes only `run`, `attach`, `peek`, `list`, and `mute`; uses argv-based tmux calls, private durable job records, bounded capture output, one-shot silence/completion notifications, and Ghostty attachment without switching the source client. It never offers session/window/pane deletion. |
