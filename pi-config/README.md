...
- `theme`: `"active"` — a stable symlink (`agent/themes/active.json`) that follows the active
  ...
  `~/.config/theme/palettes/*.json`; `agent/themes/active.json` is the **sole** managed Pi theme
...
`answer`, `ask-user`, `claude-bridge`, `diff`, `extension-toggle`, `finder`, `footer`, `header`, `image-proxy`, `lsp-startup`, `markdown-code`, `minimal-working-indicator`, `permission-gate`, `plan-mode`, `pretty`, `prompt-stash`, `protected-paths`, `save-md`, `session-recall`, `session-rename`, `session-state`, `skill-toggle`, `subagent`, `thinking-controls`, `thinking-label`, `titlebar-spinner`, `tmux`, `tool-display`, `tool-toggle`, `web-tools`, `whimsical`
<!-- /inventory:extensions -->
...
| `plan-mode/` | Structured sequential plan workflow. `Ctrl+P` cycles `none → plan → read-only → none`; restricted modes preserve bash while removing direct mutation tools. Branch navigation restores branch-local state, tools, and session model. Tmux execution uses durable claim/readiness handoffs; completed-plan retries are idempotent, and clipboard exports include the execution brief. |
...
| Extension | What it does |
|---|---|
| `footer/` | Quiet two-line footer: context usage plus model/provider/thinking, `directory (origin/branch) (worktree/name)`, and plan lifecycle status (`waiting to plan`, ready, progress, handoff, or completed session-history count). It reports auto/active compaction and uses existing `warning`/`error` semantic roles at 70%/90% context. |
...
`pretty/` is a narrow visual override for `read` and `bash` only: syntax-highlighted reads and compact bash output. Markdown reads use `markdown-code/`'s fence scanner so fenced bodies retain their literal source text and receive Pi's native theme-token syntax colors in both collapsed and expanded previews. It does not register `find`, `grep`, or `ls`.

`diff/` retains the full custom `write`/`edit`/`apply_patch` renderer, transactional patch execution, and stale-edit guard. In unified mode, multiline replacement blocks pair old/new rows by ordinal position so counterparts stay adjacent; unmatched insertions or deletions remain in source order. Split mode reserves fixed terminal-cell widths for both halves on every physical row, so one-sided changes and unequal wrapped continuations cannot collapse into the opposite column; CJK and combining characters use Pi TUI's cell-aware measurement. Regression coverage is `src/review/hunk-preview.test.ts` for renderer geometry and `src/rendering.test.ts` for the actual decorated mutation cards; from `~/.config`, run `JITI_FS_CACHE=false node --experimental-strip-types --no-warnings --test pi-config/agent/extensions/diff/src/review/hunk-preview.test.ts pi-config/agent/extensions/diff/src/rendering.test.ts`.

`tool-display/` supplies shared framing, `ctrl+shift+o` call/input-detail toggle used by multiline tool calls, and `/tool-wrap` / `ctrl+shift+w` for the **session-local** tool-output word-wrap preference. Wrapping defaults on and resets on each Pi session start/reload. When enabled, textual tool output continues across rows; when disabled, each logical output line is clipped with `…`. The setting is display-only: it never changes stored/model-visible tool results, Pi's native `ctrl+o` result-detail toggle, or `ctrl+shift+o`. It covers default-shell text results (including `find`/`grep`/`ls` and extension fallbacks), `pretty` read/bash output, and diff/review rendering; rich third-party non-text components retain their own layout unless they opt into the shared policy. A guarded host-level decorator over Pi’s `ToolExecutionComponent` adds the same status-marker prefix and call/result divider to every tool that still uses Pi’s default `Box` shell. Its hybrid visual hierarchy keeps routine cards tint-only, while `write`/`edit`/`apply_patch`, `ask_user`, and every error card replace the left frame pad with a status-colored `▌` edge (pending = warning, success = success, error = error); this keeps the `width - 2` content budget unchanged. `read` and `bash` are self-shell and render their own error edges; the diff extension provides rich call/result content inside the default `Box`, so the decorator supplies its mutation/error edge. The decorator never touches `renderShell: "self"` tools, never re-implements any tool’s renderer, and falls back to Pi’s unmodified render on any error or unrecognized component shape. Other custom tools use Pi's default shell and result presentation where the decorator applies.
...
- **Fixed vision route:** `openai-codex/gpt-5.6-sol` with high reasoning, resolved from the existing catalog — no new provider is registered and no credentials are duplicated. Sol uses the existing OpenAI Codex authentication, so the route works with no extra setup. Changing the route means editing `VISION_PROVIDER`/`VISION_MODEL_ID`/`VISION_REASONING` at the top of `image-proxy/index.ts` (no picker, no persisted config, by design).
...
- **Agents** (user-level): `scout` and `planner` use `openai-codex/gpt-5.6-luna`; `reviewer`, `researcher`, and `worker` use `openai-codex/gpt-5.6-terra`. Declared tools only narrow the parent's active policy: no agent can regain a parent-disabled tool, recursive delegation, or parent-owned plan lifecycle tools.
...
Project-local `.pi/agents/*.md` only load if a subagent call passes `agentScope: "both"` or `"project"`; they require interactive approval by default. The foreground-only runner validates one bounded mode, projects throttled child state through Pi's default tool shell, incrementally bounds output, validates terminal JSONL, terminates owned process groups, and uses mode-0600 temporary task files; see `extensions/subagent/README.md`.
