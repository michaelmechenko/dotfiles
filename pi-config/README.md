...
| `plan-mode/` | Structured plan workflow plus separate restricted access modes. `Ctrl+P` cycles `none → plan → read-only → none`; plan exposes read-only tools plus `plan_update`, while standalone read-only exposes only the shared positive allowlist. `/plan-review` executes through one toggle/settings screen with explicit Execute and Cancel actions: current session, clipboard, detached pane in the current tmux window, or detached window in the current tmux session. Tmux choices appear only when the source target resolves; detached launches preserve source focus and cwd. Model policy defaults to the saved plan default and can be changed to current or choose, with provider/model/thinking and a choose-only save-default toggle; clipboard hides model controls and changes no model state. The destination defaults to a detached pane, falling back to the current session outside tmux. `plan_step` tracks progress and `plan_complete` records closeout. The saved execution default is only `agent/plan-mode.json`'s `executionModel`; temporary model switches restore Pi global defaults. `/todos`, `/plan-edit`, `/pause`, and `/plan-widget` remain available. |
