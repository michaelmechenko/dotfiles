...
| Extension | What it does |
|---|---|
| `footer/` | Quiet two-line footer: context usage plus model/provider/thinking, `directory (origin/branch) (worktree/name)`, and plan lifecycle status (`waiting to plan`, ready, progress, handoff, or completed project-history count). It reports auto/active compaction and uses existing `warning`/`error` semantic roles at 70%/90% context. |
