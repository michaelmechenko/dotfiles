...
`diff/` retains the full custom `write`/`edit`/`apply_patch` renderer, transactional patch execution, and stale-edit guard. In unified mode, multiline replacement blocks pair old/new rows by ordinal position so counterparts stay adjacent; unmatched insertions or deletions remain in source order.
