...
| `plan-mode/` | Structured sequential plan workflow. `Ctrl+P` cycles `none → plan → read-only → none`; the restricted modes preserve the active tool baseline while disabling only `write`, `edit`, and `apply_patch`, so bash remains available. Plan mode adds `plan_update`; read-only exposes no plan tools. Valid saved execution models are selected before the session model and described with provider/model/thinking in the wizard. Successful execution retains its model. Completed plans receive stable IDs and archive atomically by canonical Git-common-dir project (or cwd fallback), so sessions and worktrees share history; the footer reports waiting, ready, executing, handoff, and completed-history state. Existing tmux handoff/recalibration behavior is retained. |
