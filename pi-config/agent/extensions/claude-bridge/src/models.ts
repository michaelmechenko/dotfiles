...
export function resolveClaudeCodeRuntimeModel(modelId: string, settings: LongContextSettings): ClaudeCodeRuntimeModel {
	switch (modelId) {
		// Opus 5 and Opus 4.8: 1M is only served with the [1m] suffix, and the
		// [1m] request on a Pro plan without usage credits enabled is rejected by
		// the subscription gateway ("credit balance is too low" / "Usage credits
		// are required for long context requests"). Max subscribers and Pro users
		// who opt into metered extra usage get [1m]; everyone else falls back to
		// the bare id, which the subscription serves at 200K.
		case "claude-opus-5":
