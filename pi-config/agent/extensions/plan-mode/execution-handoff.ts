...
export function consumeExecutionPacket(agentDir: string, handoffPath: string): ExecutionPacket | undefined {
	...
	try { const stat = lstatSync(handoffPath); if (!stat.isFile() || (stat.mode & 0o077) !== 0) return undefined; const parsed = JSON.parse(readFileSync(handoffPath, "utf8")); if (!isExecutionPacket(parsed)) return undefined; const plan = migratePlanState(parsed.plan); return plan ? { ...parsed, plan } : undefined; } catch { return undefined; } finally { if (isPrivatePath(agentDir, handoffPath)) try { unlinkSync(handoffPath); } catch { /* one-time delivery */ } }
}
