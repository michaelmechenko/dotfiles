...
export interface PlanState {
	version: 6; accessMode: AccessMode; phase: PlanPhase; goal: string; steps: TodoItem[]; criteria: string[]; followUps: string[]; executionBrief: ExecutionBrief;
	/** Assigned when the first structured update materializes this sequential plan. */
...
type LegacyPlanState = Omit<Partial<PlanState>, "version" | "executionBrief" | "phase" | "steps"> & { version?: unknown; phase?: string; steps?: Array<Partial<TodoItem>> };
...
export function createPlanState(): PlanState { return { version: 6, accessMode: "none", phase: "idle", goal: "", steps: [], criteria: [], followUps: [], executionBrief: emptyExecutionBrief(), widgetCollapsed: true, awaitingReview: false, resumeAfterRevision: false, completionRequested: false }; }

/** Assign stable identity exactly once, at the first structured plan update. */
export function materializePlan(state: PlanState, now = new Date().toISOString()): PlanState {
	if (!state.steps.length || state.planId) return state;
	return { ...state, planId: randomUUID(), createdAt: now };
}

/** v2-v5 plans receive deterministic step IDs; obsolete parallel state is discarded. */
export function migratePlanState(data: unknown): PlanState | undefined {
	if (!data || typeof data !== "object") return undefined;
	const legacy = data as LegacyPlanState;
	if (legacy.version === 6) return isPlanState(legacy) ? clonePlanState(legacy) : undefined;
	if (legacy.version !== 2 && legacy.version !== 3 && legacy.version !== 4 && legacy.version !== 5) return undefined;
	...
	const migrated: PlanState = { ...createPlanState(), ...legacyFields, version: 6, steps, phase: isPlanPhase(phase) ? phase : "idle", accessMode: legacy.version === 2 && (legacy.phase === "planning" || legacy.phase === "revising") ? "plan" : isAccessMode(legacy.accessMode) ? legacy.accessMode : "none", executionBrief: legacy.version !== 2 && legacy.version !== 3 && isExecutionBrief((legacy as { executionBrief?: unknown }).executionBrief) ? cloneExecutionBrief((legacy as { executionBrief: ExecutionBrief }).executionBrief) : emptyExecutionBrief() };
