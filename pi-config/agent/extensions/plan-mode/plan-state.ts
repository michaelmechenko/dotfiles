...
export interface ExecutionBrief {
	...
	findings: string[];
	decisions: string[];
	relevantFiles: { path: string; note: string }[];
	constraints: string[];
}
...
type LegacyPlanState = Omit<Partial<PlanState>, "version" | "executionBrief"> & { version?: unknown; phase?: string };

export function emptyExecutionBrief(): ExecutionBrief {
	return { summary: "", findings: [], decisions: [], relevantFiles: [], constraints: [] };
}

export function createPlanState(): PlanState {
	return { version: 4, accessMode: "none", phase: "idle", goal: "", steps: [], criteria: [], followUps: [], executionBrief: emptyExecutionBrief(), widgetCollapsed: true, awaitingReview: false, resumeAfterRevision: false, completionRequested: false };
}

/** Validates every persisted field. v2/v3 plans retain an empty brief until recalibration replaces it. */
export function migratePlanState(data: unknown): PlanState | undefined {
	if (!data || typeof data !== "object") return undefined;
	const legacy = data as LegacyPlanState;
	if (legacy.version === 4) return isPlanState(legacy) ? clonePlanState(legacy) : undefined;
	if (legacy.version !== 2 && legacy.version !== 3) return undefined;
	const phase = legacy.phase === "planning" ? "drafting" : legacy.phase ?? "idle";
	const migrated: PlanState = {
		...createPlanState(),
		...legacy,
		version: 4,
		phase: isPlanPhase(phase) ? phase : "idle",
		accessMode: legacy.version === 2 && (legacy.phase === "planning" || legacy.phase === "revising") ? "plan" : isAccessMode(legacy.accessMode) ? legacy.accessMode : "none",
		executionBrief: emptyExecutionBrief(),
	};
...
export function enterRestrictedMode(state: PlanState, accessMode: Exclude<AccessMode, "none">, activeTools: string[]): PlanState {
	...
	} else if (state.accessMode === "plan" && (phase === "drafting" || phase === "ready" || phase === "revising")) phase = "paused";
...
export function leaveRestrictedMode(state: PlanState): { state: PlanState; restoreTools?: string[] } {
	return { state: { ...state, accessMode: "none", toolsBeforePlan: undefined }, restoreTools: state.toolsBeforePlan ? [...state.toolsBeforePlan] : undefined };
}
...
export function isExecutionBrief(value: unknown): value is ExecutionBrief {
	if (!value || typeof value !== "object") return false;
	const brief = value as Partial<ExecutionBrief>;
	return typeof brief.summary === "string" && isStringArray(brief.findings) && isStringArray(brief.decisions) && isStringArray(brief.constraints)
		&& Array.isArray(brief.relevantFiles) && brief.relevantFiles.every((file) => !!file && typeof file === "object" && typeof (file as { path?: unknown }).path === "string" && typeof (file as { note?: unknown }).note === "string");
}
export function isPlanState(value: unknown): value is PlanState {
	if (!value || typeof value !== "object") return false;
	const plan = value as Partial<PlanState>;
	return plan.version === 4 && isAccessMode(plan.accessMode) && isPlanPhase(plan.phase) && typeof plan.goal === "string"
		&& isTodoList(plan.steps) && isStringArray(plan.criteria) && isStringArray(plan.followUps) && isExecutionBrief(plan.executionBrief)
		&& (plan.executionSource === undefined || isExecutionSource(plan.executionSource))
		&& (plan.toolsBeforePlan === undefined || isStringArray(plan.toolsBeforePlan))
		&& (plan.planningModel === undefined || isModelSnapshot(plan.planningModel)) && (plan.executionModel === undefined || isModelSnapshot(plan.executionModel))
		&& typeof plan.widgetCollapsed === "boolean" && typeof plan.awaitingReview === "boolean" && typeof plan.resumeAfterRevision === "boolean" && typeof plan.completionRequested === "boolean"
		&& (plan.closeout === undefined || isCloseout(plan.closeout));
}
function clonePlanState(state: PlanState): PlanState { return { ...state, steps: state.steps.map((step) => ({ ...step })), criteria: [...state.criteria], followUps: [...state.followUps], executionBrief: cloneExecutionBrief(state.executionBrief), executionSource: state.executionSource ? { ...state.executionSource } : undefined, toolsBeforePlan: state.toolsBeforePlan ? [...state.toolsBeforePlan] : undefined, planningModel: state.planningModel ? { ...state.planningModel } : undefined, executionModel: state.executionModel ? { ...state.executionModel } : undefined, closeout: state.closeout ? { ...state.closeout, verification: [...state.closeout.verification], deviations: [...state.closeout.deviations], nextSteps: [...state.closeout.nextSteps] } : undefined }; }
...
function isTodoList(value: unknown): value is TodoItem[] { return Array.isArray(value) && value.every((step) => !!step && typeof step === "object" && Number.isInteger((step as TodoItem).step) && typeof (step as TodoItem).text === "string" && typeof (step as TodoItem).completed === "boolean" && typeof (step as TodoItem).skipped === "boolean"); }
