...
assert.match(hunkPreview, /import \{ truncateToWidth, visibleWidth, wrapTextWithAnsi \} from "@earendil-works\/pi-tui";/, "production diff layout must use terminal-cell-aware TUI primitives");
assert.match(hunkPreview, /function composeHalf\(half: HalfResult, rowIndex: number\)/, "split continuation rows must preserve fixed half widths");
assert.doesNotMatch(hunkPreview, /leftBody\.trimEnd\(\).*rightBody\.trimEnd\(\)/, "split composition must not remove layout-significant padding");
