...
const DEFAULTS = {
	...
	searchEnabled: true,
	searchProvider: "exa",
	// Exa's official hosted MCP endpoint. Requires an EXA_API_KEY (sent as the `x-api-key` header,
	// see providers/exa.ts). Forked from dmmulroy's config, which pointed this at his personal
	// proxy (https://m.mulroy.dev/m/e) instead of calling Exa directly with a caller-owned key.
...
/** Read EXA_API_KEY from the environment, wrapped so it never gets logged/serialized accidentally. */
function readExaApiKey(): ReturnType<typeof Redacted.make<string>> | undefined {
