...
#   floating → #8b8494, tiling/"sticky" → #75758D. Fullscreen keeps its layout color (no special
#   tint) — only the SketchyBar name brackets mark fullscreen.
...
CUR_COLORS="$(printf '%s\n' "$WINDOW_DATA" | awk -F'\t' '
  NF>=6 && $3!="" {
    c="0xff75758D"                                  # tiling / sticky
    if ($6=="floating") c="0xff8b8494"              # floating
    print $3" "c
  }' | sort)"
