...
# Stable Home Manager links prevent long-lived graphical sessions from retaining
# obsolete Nix store paths after a generation switch.
if [[ $OSTYPE == linux* && -d "$HOME/.config/oh-my-zsh" ]]; then
...
# fzf colors + moor hint → active palette (generated by `theme switch`).
# bg/bg+/gutter/preview-bg = -1 (transparent). Left bar indicator via pointer=▌.
# Match hierarchy: hl dusty-pink (other rows), hl+ rose bold (current row).
[[ -r ~/.config/theme/active/shell/palette.sh ]] && source ~/.config/theme/active/shell/palette.sh
...
if command -v moor >/dev/null 2>&1; then
    export NNN_PAGER="moor --no-linenumbers --mousemode=scroll --statusbar=plain --scroll-left-hint=${THEME_MOOR_HINT}<ESC[39m --scroll-right-hint=${THEME_MOOR_HINT}>ESC[39m"
else
...
if [[ $OSTYPE == darwin* ]]; then
  alias lg="lazygit --use-config-file ~/.config/theme/generated/lazygit/config.yml"
else
  alias lg="lazygit"
fi
alias ld="lazydocker"
...
t() {
  ...
  if (( $(echo "$ver < 3.7" | bc -l) )); then
    echo "tmux 3.7+ is required for the -O session-switch flags; install the stable formula with brew install tmux. This config also supports a tested next-3.8 HEAD keg only on an isolated socket until popup validation passes; keep 3.7b as the live default."
    return 1
  fi
...
[[ $OSTYPE == darwin* ]] && alias bu="brew update && brew upgrade"
alias c="clear"
alias k="kubectl"
alias tf="terraform"
...
if [[ $OSTYPE == darwin* ]]; then
  eval "$(oh-my-posh init zsh --config ~/.config/theme/generated/ohmyposh/theme.json)"
else
...
if [[ $OSTYPE == darwin* ]] && \
  [[ "$(defaults read org.hammerspoon.Hammerspoon MJConfigFile 2>/dev/null)" != "$HOME/.config/hammerspoon/init.lua" ]]; then
